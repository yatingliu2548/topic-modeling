...
for (exp in 1:50){
  for (nb_samples in c(5000, 10000)){
    ...
    for (k in 3:18){
      it = 1
        alignment <- align_topics(exp(topic_models_ours[[paste0("k",k)]]$beta),
                                  exp(topic_models_ours_test[[paste0("k",k)]]$beta), 
      ...
      res_temp = data.frame("min"=min(diag(as.matrix(match_permuted))),
                           ...
                           "m" = nb_samples,
                            "off_diag" = (sum(match_permuted) - sum(diag(as.matrix(match_permuted))))/(ncol(match_permuted)^2 - ncol(match_permuted)) )
      res = rbind(res, res_temp)
      write_csv(res, "~/Documents/topic-modeling/r/experiments/real/spleen_results_final_l1_dist.csv")
