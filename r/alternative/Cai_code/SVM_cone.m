function [M1,pure,Z,yp] = SVM_cone(Z,k,deg,topic_flag,beta,clambda,sa)
%
...
% Least square
 M1 = Z/yp;
% NNLS of M
% [M,~]= nnls(Z,yp,struct('Accy',0))
%tic;
%M2=zeros(size(Z,1),k);
%for L=1:size(Z,1)
%   vec=(nnls(yp',Z(L,:)'))'; 
   %vec=(lsqnonneg(yp',Z(L,:)'))';
   %threshold=min(maxk(vec,sa));
   %vec(vec<threshold)=0;
  %M2(L,:)=vec;
%end
