...
def do_recovery(Q, anchors, loss, log_prefix, max_threads, eps):
    if loss == "originalRecover":
        return (Recover(Q, anchors), None)
    elif loss == "KL" or "L2" in loss:
        A, colsums = nonNegativeRecover(Q, anchors, divergence=loss, max_threads=max_threads,initial_stepsize=1,  epsilon=eps)
        hp = colsums
...
def nonNegativeRecover(Q, anchors, divergence, max_threads, initial_stepsize=1, epsilon=10**(-7)):
    ...
    initial_stepsize = int(initial_stepsize)
    V = Q.shape[0]
    K = len(anchors)
    A = matrix(zeros((V, K)))
    ...
    if max_threads > 0:
        ...
        args = myIterator(Q, anchors, "out.recoveryLog", divergence, V, initial_stepsize, epsilon)
        ...
        if divergence == "gurobi_L2":
            ...
            for w in range(V):
                y = Q[w, :]
                v, it, obj, alpha, stepsize, t, gap= fastRecover((y,X,w,"out.recoveryLog",anchors,divergence,XXT,initial_stepsize, epsilon))
                A[w, :] = alpha
                # if v % 1 == 0:
